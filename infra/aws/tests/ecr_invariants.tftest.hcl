variables {
  alert_email = "alerts@example.invalid"
}

# Image repos hold content-addressed <sha> tags: immutability makes a tag a
# permanent name for one image, and scan-on-push is free table stakes. The
# repos now come from the ecr-repo module, so the invariants read its outputs.
run "ecr_repos_are_immutable_and_scanned" {
  command = plan

  assert {
    condition = alltrue([
      module.caddy.image_tag_mutability == "IMMUTABLE",
      module.hello.image_tag_mutability == "IMMUTABLE",
    ])
    error_message = "ECR tags must be immutable; tags are content-addressed shas."
  }

  assert {
    condition = alltrue([
      module.caddy.scan_on_push,
      module.hello.scan_on_push,
    ])
    error_message = "ECR repos must scan on push."
  }

  assert {
    condition = alltrue([
      module.caddy.repository_name == "wkx/caddy",
      module.hello.repository_name == "wkx/hello",
    ])
    error_message = "ECR repos follow the wkx/<service> naming pattern."
  }
}
