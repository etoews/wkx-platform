...
This repo has left the pure design phase. Live today: `infra/` (four Terraform roots: `bootstrap/`, `aws/`, `cloudflare/`, `mgmt/`, all applied; M1 network, M2 Graviton host, and the management-account budgets), `host/cloud-init.yaml` (the cloud Host bootstrap; installs git + aws-cli as of M3), and `platform/` plus `hello/` (the Platform stack: Caddy behind Cloudflare with one wildcard cert, flat snippet dir; and the `hello` smoke-test app; M3 complete). Still to come milestone by milestone per `ROADMAP.md`: Python tooling under `tools/` (M5 onward) and the reference project under `template/` (M8). The design spec, milestone plans, ADRs under `docs/adr/`, and the `CONTEXT.md` glossary remain the sources of truth.
