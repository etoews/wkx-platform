...
- **Data volume:** `aws_ebs_volume.data`, 20 GB gp3, encrypted, same AZ, `prevent_destroy`, attached as `/dev/sdf`. Independent of the instance's lifecycle; this is what M10 snapshots. The attachment sets `stop_instance_before_detaching = true`, so a replacement cleanly unmounts the filesystem rather than force-detaching a live volume.
...
3. **Data volume format-if-blank + mount**, via a single idempotent `runcmd` script rather than the `fs_setup`/`mounts` modules. Those modules run in cloud-init's early init stage, seconds into first boot, while Terraform cannot call `AttachVolume` until the instance reports `running`; the device would usually not exist yet, the format step would be silently skipped, and the Host would come up with `/srv/data` as a bare directory. The `runcmd` script (final cloud-init stage) instead: polls for the stable `/dev/disk/by-id/nvme-Amazon_Elastic_Block_Store_vol...` symlink (derived from the templated volume ID, not the racy `/dev/sdf` name) for up to 3 minutes; runs `mkfs.ext4 -L wkx-data` only if `blkid` reports no filesystem on the device; appends `LABEL=wkx-data /srv/data ext4 defaults,nofail 0 2` to fstab if absent; mounts; and hands ownership of `/srv/data` to the `platform` user. On a replacement instance the existing filesystem is detected and simply mounted; every step is a no-op when its work is already done.
